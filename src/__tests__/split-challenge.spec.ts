...
describe('split-challenge', () => {
  ...
  describe('getSplitConfig', () => {
    ...
    it('must return A or B split config when cookie is not set', () => {
      ;(parseCookies as jest.Mock).mockReturnValue({})
      expect(
        getSplitConfig({} as GetServerSidePropsContext, 'test1')
      ).toMatchObject({
  ...
  describe('getPath', () => {
    it('must return path string when using ":path*"', () => {
      expect(
        getPath({ path: '/foo/:path*' } as SplitConfig, {
          path: ['foo', 'bar']
        })
      ).toEqual('/foo/foo/bar')
      expect(
      ...
      expect(
        getPath({ path: '/foo/:path*/bar' } as SplitConfig, {
          path: ['foo', 'bar']
        })
      ).toEqual('/foo/foo/bar/bar')
      expect(getPath({ path: '/foo/:path*' } as SplitConfig, {})).toEqual(
        '/foo/'
      )
      expect(
        getPath({ path: '/foo/:path*' } as SplitConfig, { path: 'bar' })
      ).toEqual('/foo/bar')
    })
    it('must return path string when using regex conditions', () => {
      expect(
        getPath({ path: '/foo/bar-:id(\\d+)' } as SplitConfig, { id: '123' })
      ).toEqual('/foo/bar-123')
      expect(
        getPath({ path: '/foo/bar-:id(\\d+)/xyz-:slug(\\w+)' } as SplitConfig, {
          id: '123',
          slug: 'xyz'
        })
    ...
    it('must return path string when using complex conditions', () => {
      expect(
        getPath(
          {
            path: '/foo/bar-:id(\\d+)/xyz-:slug(\\w+)/country-:country(japan|america)/:path*'
          } as SplitConfig,
          {
            id: '123',
            slug: 'xyz',
  ...
  describe('sticky', () => {
    it('must call', () => {
      ...
      expect(setCookie).toBeCalledWith({}, 'x-split-key-test1', 'branch1', {
        path: '/',
        maxAge: 60
      })
    })
